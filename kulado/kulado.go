...
// LoadBalancer returns an implementation of LoadBalancer for Kulado
func (r *CloudProvider) LoadBalancer() (cloudprovider.LoadBalancer, bool) {
	return r, true
}

// Zones returns an implementation of Zones for Kulado
func (r *CloudProvider) Zones() (cloudprovider.Zones, bool) {
	return r, true
}

// Instances returns an implementation of Instances for Kulado
...
func (r *CloudProvider) EnsureLoadBalancer(clusterName string, service *api.Service, nodes []*api.Node) (*api.LoadBalancerStatus, error) {
	...
	if loadBalancerIP != "" {
		// Kulado doesn't support specifying loadBalancer IP
		return nil, fmt.Errorf("loadBalancerIP cannot be specified for Kulado LoadBalancer")
	}

	if affinity != api.ServiceAffinityNone {
		// Kulado supports sticky sessions, but only when configured for HTTP/HTTPS
...
func (r *CloudProvider) NodeAddresses(nodeName types.NodeName) ([]api.NodeAddress, error) {
	...
	addresses = append(addresses, api.NodeAddress{
		...
		Address: host.KuladoHost.Hostname,
	})
...
func (r *CloudProvider) InstanceID(nodeName types.NodeName) (string, error) {
	...
	return host.KuladoHost.Uuid, nil
}
...
func (r *CloudProvider) getHostByName(name string) (*Host, error) {
	...
	hostname := name
	for _, host := range hosts.Data {
		kuladoFQDNParts := strings.Split(host.Hostname, ".")
		kuladoHostname := host.Hostname
		if len(kuladoFQDNParts) > 1 {
			// kulado uses fqdn
			if len(fqdnParts) == 1 {
				// truncate kulado fqdn to hostname
				// if kulado uses fqdn but kubelet
				// uses hostname
				kuladoHostname = kuladoFQDNParts[0]
			}
		} else {
			// kulado uses hostname
			hostname = fqdnParts[0]
		}
		if strings.EqualFold(kuladoHostname, hostname) {
	...
	kuladoHost := &hostsToReturn[0]

	coll := &client.IpAddressCollection{}
	err = r.client.GetLink(kuladoHost.Resource, "ipAddresses", coll)
	if err != nil {
...
func newKuladoCloud(config io.Reader) (cloudprovider.Interface, error) {
	...
	client, err := getKuladoClient(conf)
	if err != nil {
		return nil, fmt.Errorf("Could not create kulado client: %#v", err)
	}
...
func hostStoreKeyFunc(obj interface{}) (string, error) {
	return obj.(*Host).KuladoHost.Hostname, nil
}

func getKuladoClient(conf rConfig) (*client.KuladoClient, error) {
...
func metadata(path string) (string, error) {
	resp, err := http.Get("http://kulado-metadata/latest" + path)
	if err != nil {
