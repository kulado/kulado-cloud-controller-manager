...
func generateCollectionResourcePages() error {
	...
	apiNavBarName := "kulado-api-sidebar-" + *version + ".html"
	...
	for fieldName, field := range schema.ResourceFields {
		...
		if includeAPIField {
			...
			if referenceRegexp.MatchString(field.Type) {
				...
				apiField.TypeURL = "[" + field.Type + "]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/api-resources/" + field.Type + "/)"
				apiField.TypeHTMLURL = `<a href="/kulado/v1.2/en/api/api-resources/` + field.Type + `/">` + field.Type + `</a>`
			}
...
func getRefTypeURL(input string) string {
	return referenceRegexp.ReplaceAllString(input, "[$1]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/api-resources/$1/)")
}

func getRefTypeHTMLURL(input string) string {
	return referenceRegexp.ReplaceAllString(input, `<a href="{{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/api-resources/$1/">$1</a>`)
}
...
func getTypeURL(typeInput string) string {
	...
	if _, isResourceType := schemaMap[resourceName]; isResourceType {
		urlResourceName := "[" + resourceName + "]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/api-resources/" + resourceName + "/)"
...
func getTypeHTMLURL(typeInput string) string {
	...
	if _, isResourceType := schemaMap[resourceName]; isResourceType {
		urlResourceName := `<a href="/kulado/v1.2/en/api/api-resources/` + resourceName + `/">` + resourceName + `</a>`
		return strings.Replace(typeInput, resourceName, urlResourceName, -1)
	}
	return typeInput
}
